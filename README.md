...
Скрипт добавляет всплывающее окно поиска (вызывается через Alt+S) и кнопки на сайтах IMDb и Кинопоиск для перекрёстного поиска. Позволяет быстро найти открытый фильм или сериал на соседней платформе (с Кинопоиска на IMDb и наоборот), автоматически подбирая правильный поисковый запрос по оригинальному названию и году выхода.
...
Добавляет кнопки SteamDB и PCGamingWiki. Отображает актуальный онлайн игроков и вычисляет точный рейтинг по алгоритму SteamDB.